...
Bool HandleError(void *pCodec)
{
    ...
    if (error) {
        sqlite3ErrorWithMsg((sqlite3*)GetDB(pCodec), SQLITE_ERROR, "AES Error: %s", error);
        return 1;
    }
    return 0;
}
...
int sqlite3_rekey(sqlite3 *db, const void *zKey, int nKey)
{
    ...
    if (rc == SQLITE_OK)
    {
        ...
        for (n = 1; rc == SQLITE_OK && n <= nPage; n++)
        {
            ...
            rc = sqlite3PagerGet(pPager, n, &pPage, 0);     // AANPASSING - ClrFlags = 0.
    ...
    else
    {
        ...
        sqlite3BtreeRollback(pbt, SQLITE_ERROR, 0); // AANPASSING - writeOnly = 0.
