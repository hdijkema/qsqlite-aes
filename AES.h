...
typedef quint32     aes_uint32_t;
typedef quint8      aes_byte_t;
typedef quint16     aes_uint16_t;
typedef qint32      aes_int32_t;
...
public:
    ...
    void StartEncryption(const aes_byte_t * key);

    // encrypt a single block (default 128 bits, or aes_byte_t[16]) of data
    void EncryptBlock(const aes_byte_t * datain, aes_byte_t * dataout);
    ...
    void Encrypt(const aes_byte_t * datain, aes_byte_t * dataout, aes_uint32_t numBlocks, BlockMode mode = CBC);

	// call this before any decryption with the key to use
    void StartDecryption(const aes_byte_t * key);

    // decrypt a single block (default 128 bits, or aes_byte_t[16]) of data
    void DecryptBlock(const aes_byte_t * datain, aes_byte_t * dataout);
    ...
    void Decrypt(const aes_byte_t * datain, aes_byte_t * dataout, aes_uint32_t numBlocks, BlockMode mode = CBC);
...
private:
    ...
    aes_byte_t W[4*8*15];   // the expanded key

	// Key expansion code - makes local copy
    void KeyExpansion(const aes_byte_t * key);
...
/* PADDING:
    ...
    3. Each padding byte is set to a random value. The decryptor must know how many bytes are in
...
The Encrypt() function is used to encrypt larger blocks of data. The block size has to be a multiple
of the method's block size. This function can operate in the following modes: ECB, CBC or CFB.
ECB mode is not using chaining. If the same block is encrypted twice with the same key, the
resulting ciphertext blocks are the same. In CBC mode, a ciphertext block is obtained by first XORing
the plaintext block with the previous ciphertext block, and encrypting the resulting value. In CFB mode,
a ciphertext block is obtained by encrypting the previous ciphertext block and XORing the resulting
value with the plaintext. The operation mode is specified in the iMode parameter with ECB being the default value.
*/
